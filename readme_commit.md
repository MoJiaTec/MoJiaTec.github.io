...
## 2.1 对象复杂度评估
&emsp;&emsp;本方案，对于复杂度的评估，基于简化后的模型，主要包含网格(Mesh)，纹理，材质三个要素，他们的具体数据可以在烘焙特定平台资源的时候可以得到。其中：<br>
...
## 4.1 复杂度控制模块
&emsp;&emsp;在游戏运行时，当场景对象确定之后，复杂度控制的本质就是如何在设定的策略和有限的硬件资源条件下，尽可能多的显示更多更好的物件，即在性能和画面之间取得一个较好的平衡。在我们方案中，我们通过智能控制场景中对象的加载卸载、显示隐藏、LOD控制等手段，获取平滑FPS。因此，我们方案的复杂度控制模块，会接受三个类型的控制指令：调整LOD、显示隐藏对象、加载卸载对象。这三条指令的调整力度依次增强。其中：<br>
...
![ObjectPool\label{fig:ObjectPool}](ObjectPool.png)
