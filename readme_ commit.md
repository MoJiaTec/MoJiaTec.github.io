...
# 2.输入模块
&emsp;&emsp;场景的复杂度，是场景中所有对象复杂度的总和。场景对象的复杂度，主要由Mesh，纹理，材质等核心要素决定，但在确定物件的物理复杂度之后，我们还要根据物件的距离(Distance)、屏幕占比(ScreenSize)、重要程度(根据需要人为定义，比如社交属性等)等因素计算物件的一个份量(Bucket)。<br>
